...
int main(int Parameter_count, char *Parameters[]) {
    ...
    while (true) {
        ...
        if ((Header & 1) == 0) {
            ...
            if (Location < Chunks.size()) { // defensive programming to avoid
                                            // out-of-bounds reference
                const std::string &Chunk = Chunks[Location];
                std::cout << "Found chunk of size " << Chunk.length()
                          << " in database.\n";
                Output.write(&Chunk[0], Chunk.length());
            } else {
                std::cerr << "Location " << Location
                          << " not in database of length " << Chunks.size()
