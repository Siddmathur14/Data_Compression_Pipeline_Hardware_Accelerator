...
#define c8mix(in)                                                              \
    ...
    const uint32_t key4[4] = {((key)&0xFF), ((key >> 8) & 0xFF),
                              ((key >> 16) & 0xFF), ((key >> 24) & 0xFF)};
    uint32_t h = SEED_CRAP, k = n;
    uint64_t p;

    c8mix(key4[0]) c8mix(key4[1]) c8mix(key4[2])
        c8mix(key4[3] & ((1 << (2 * 8)) - 1)) c8fold(h ^ k, n, k, k) return k;
}
...
unsigned int my_hash(unsigned long key) {
    ...
    // return ((((hash_2 >> 3) & 0x3F) << 7) | (((hash_3 >> 5) & 0x7) << 4) |
    // ((hash_1 >> 7) & 0xF)); return ((((hash_2 >> 3) & 0x7F) << 7) |  ((hash_1
    // >> 7) & 0x7F)); return ((hash_1 + (SEED_CRAP ^ hash_2)) >> 7) &
    // (CAPACITY_MOD);
...
void inline hash_insert(unsigned long (*hash_table)[2], unsigned int key,
    ...
    if (!valid) {
        hash_table[hash_val][0] =
            (1UL << (21 + 13)) | ((unsigned long)(value) << 21) | key;
        *collision = 0;
        return;
    }
...
static void store_data(hls::stream<uint32_t> &out_stream, uint32_t *lzw_codes) {
    unsigned int i = 0;
...
void lzw(unsigned char input[BUFFER_LEN], uint32_t *lzw_codes,
